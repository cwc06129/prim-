...
char *EXIT_STRING = "exit"; // 채팅 프로그램을 나가고 싶을 경우, 입력하는 문자열

char *START_STRING = "Good afternoon! Any question? \n";
...
int main(int argc, char *argv[]) 
{
	...
	fd_creat = creat("./primae.txt", 0644);
	if (fd_creat == -1)
		printf("create error");
	...
	while (1)
	{
		...
		fd_open = open("./primae.txt", O_RDWR);
		if (fd_open == -1)
		{
			printf("open error");
			return 1;
